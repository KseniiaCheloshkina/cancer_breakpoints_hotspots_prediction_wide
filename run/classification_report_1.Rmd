...
ci_stats <- all_features_data %>%
  ...
  mutate(
    ci_lower_recall = mean_recall - 1.96 * sd_recall / n_ex,
    ci_upper_recall = mean_recall + 1.96 * sd_recall / n_ex,
    
    ci_lower_lift_recall = mean_lift_recall - 1.96 * sd_lift_recall / n_ex,
    ci_upper_lift_recall = mean_lift_recall + 1.96 * sd_lift_recall / n_ex,    
    
    ci_lower_precision = mean_precision - 1.96 * sd_precision / n_ex,
    ci_upper_precision = mean_precision + 1.96 * sd_precision / n_ex,       
    
    ci_lower_lift_precision = mean_lift_precision - 1.96 * sd_lift_precision / n_ex,
    ci_upper_lift_precision = mean_lift_precision + 1.96 * sd_lift_precision / n_ex
  )
...
Comparison of median lift of recall and median recall for 0.05 probability quantiles for task of hotspots prediction and breakpoints prediction

```{r}
agg_recall_all_agg <- recall_comp %>%
  filter(quantile == 0.05) %>%
  group_by(cancer_type, type) %>%
  summarize(
...
agg_recall_all_agg_1 <- agg_recall_all_agg %>%
  filter(type == 'hotspots') %>%
  select(cancer_type, med_recall, med_lift_recall) %>%
  setNames(c("cancer_type", "med_recall_hsp", "med_lift_recall_hsp"))

agg_recall_all_agg_2 <- agg_recall_all_agg %>%
  filter(type == 'breakpoints') %>%
  select(cancer_type, med_recall, med_lift_recall) %>%
  setNames(c("cancer_type", "med_recall_bkpt", "med_lift_recall_bkpt"))

comp_best_all <- agg_recall_all_agg_2 %>%
  inner_join(agg_recall_all_agg_1, by=c("cancer_type"))

comp_best_all %>%
  select("cancer_type", "med_recall_hsp","med_recall_bkpt","med_lift_recall_hsp", "med_lift_recall_bkpt")
...
recall_stats_bkpt <- ci_stats_bkpt %>% 
  ungroup() %>%
  filter(quantile %in% c(0.001, 0.005,0.01,0.02,0.03, 0.040, 0.050, 0.100)) %>%
  select(cancer_type, quantile, mean_recall, mean_lift_recall)
...
saveWorkbook(wb, "../reports/train_baseline.xlsx", overwrite = T)
```
