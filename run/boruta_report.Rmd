...
ggplot(df_hits_last_iter_agg_new, aes(x=cancer_type, y=n_best, fill=feature_group))+
  geom_bar(stat="identity", position="dodge") + 
  scale_fill_jco()
```
...
```{r}
thr <- df_hits_last_iter_agg %>%
  group_by(cancer_type) %>%
  summarize(
    thr = quantile(n_best, .85)
  )
df_hits_last_iter_agg %>%
  inner_join(thr, by=c("cancer_type")) %>%
  filter(n_best > thr) %>%
  group_by(cancer_type) %>%
  summarize(
    n_features = n()
...
saveWorkbook(wb, "../data/output/boruta_stats_0.8.xlsx", overwrite = T)
```
