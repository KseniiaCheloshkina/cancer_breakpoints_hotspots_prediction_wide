...
```
```{r}
comp_rec_pl$variable_type[comp_rec_pl$variable_type == 'recall'] <- 'lift_recall'
comp_rec_pl$variable_type[comp_rec_pl$variable_type == 'precision'] <- 'lift_precision'
g_boruta <- ggplot(comp_rec_pl, aes(x=cancer_type, y=value, fill=variable_new)) + 
  geom_bar(stat='identity',  color="black", position = "dodge") +
  facet_wrap(~variable_type,nrow = 2) + 
  theme_light_custom()+
  xlab("Cancer type") + 
