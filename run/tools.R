...
limit_outliers <- function(x_train, x_test, features, iqr_multiplicator = 3){
  ...
  for (col in features){
    ...
    x_train_tr[x_train_tr[col] < as.numeric(borders[[col]]['lower_border']), col] <- as.numeric(borders[[col]]['lower_border'])
    x_train_tr[x_train_tr[col] > as.numeric(borders[[col]]['upper_border']), col] <- as.numeric(borders[[col]]['upper_border'])
    
    x_test_tr[x_test_tr[col] < as.numeric(borders[[col]]['lower_border']), col] <- as.numeric(borders[[col]]['lower_border'])
    x_test_tr[x_test_tr[col] > as.numeric(borders[[col]]['upper_border']), col] <- as.numeric(borders[[col]]['upper_border'])
  }
