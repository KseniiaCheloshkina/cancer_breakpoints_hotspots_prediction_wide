...
for_plot_final <- rbind.data.frame(
  roc_data_comp_m %>% select(cancer_type, value, variable, type)%>% filter(variable == 'ROC AUC'),
  comp_rec_pl %>% select(cancer_type, value, variable, type) %>% filter(variable == 'Lift of recall')
)
...
g_f
# ggsave(filename = "FigS13.tiff", plot = g_f, device = "tiff", path = "../paper_materials/supplemental/",
