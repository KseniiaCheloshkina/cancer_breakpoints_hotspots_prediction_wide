...
```{r fig.width=10}
ggplot(recall_ci_stats_bl %>%filter(quantile_chr != "0.15"), 
...
ggplot(recall_ci_stats_bl_comp, aes(x=quantile_chr, y=mean_lift_recall_diff, fill=type)) + 
  geom_bar(stat="identity", color="black", position=position_dodge()) +
...
```{r fig.width=10}
ggplot(roc_auc_ci_stats_bl_all, aes(x=eps, y=mean_roc_auc_diff, fill=type)) + 
  geom_bar(stat="identity", color="black", position=position_dodge()) +
  geom_errorbar(aes(ymin=ci_lower_roc_auc_diff, ymax=ci_upper_roc_auc_diff), width=.2, position=position_dodge(.9))+
  scale_fill_jco()+
  facet_wrap(~cancer_type, nrow = 5, scales = "free")
...
writeData(wb, sheet="Recall_RN", rec_final_rp)
