############# STEPWISE ONE-BY-ONE FEATURE ADDITION TO BORUTA FEATURE SET
...
output_path <- "data/output/prostate_fs/" 
...
# target_column <- "hsp_99.9._pancreatic"
# target_column <- "hsp_99.5._breast"
# target_column <- "hsp_99._prostate"
...
feat_to_check <- setdiff(all_features_cols, features_nm)
...
if (length(features_nm) + 1 < 5){
  mtryGrid <- expand.grid(mtry = length(features_nm) + 1)
} else {
  mtryGrid <- expand.grid(mtry = 5)
}

# set progress bar
n_iterations <- length(feat_to_check)
pb <- progress_bar$new(
...
for (feat in feat_to_check){
  new_features_nm <- c(features_nm, feat)
  repeats_res <- foreach(i=seq(1, n_repeats)) %dopar% {
    ...
    splitted_dataset <- get_train_test_split(data=all_data, target_col=target_column, start_pos_col="from",
                                             chr_col="chr", feature_cols=new_features_nm, train_ratio=train_ratio, 
                                             seed=i)
    ...
    # fit models
    model <- rf_fit(x_train = x_train[new_features_nm], y_train = y_train, trCtrl = trCtrl,
                    n_pos = n_pos, n_neg = n_neg, mtryGrid = mtryGrid)
    
    # prediction
    train_pred <- predict(model, newdata = x_train[new_features_nm], type = "prob")
    test_pred <- predict(model, newdata = x_test[new_features_nm], type = "prob")
