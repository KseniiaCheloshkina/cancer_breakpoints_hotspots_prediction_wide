...
```{r fig.width=16}
roc_auc_data_cur <- roc_auc_data_all[roc_auc_data_all$type != "hsp_vs_bkpt",]
recall_data_cur <- recall_data_all[recall_data_all$type != "hsp_vs_bkpt",]
...
g1 <- ggplot(roc_auc_data_cur, aes(x=agg_level, y=te_auc, fill=agg_level)) + 
  ...
  xlab("Hotspots labeling threshold") + 
  ylab("ROC AUC") +
  theme(
    ...
    legend.position="none", legend.title = element_text(size = 10),
    ) + 
  labs(fill = "Threshold")
g1
```
...
g3 <- ggplot(roc_auc_data_base_all, aes(x=agg_level, y=te_auc, fill=type1)) + 
  geom_boxplot() + 
...
Cases when lift of recall for breakpoints prediction models is significantly lower than for hotspots prediction models 
...
ggplot(recall_data_base_all %>%
  filter(
    ...
    cancer_type %in% c("breast", "pancreatic", "prostate", "ovary")
    ), 
  aes(x=agg_level, fill=type, y=lift_recall)) + 
  geom_boxplot() + 
  facet_grid(quantile ~ cancer_type, scales = "free_y") + 
...
Cases when lift of recall for breakpoints prediction models is not significantly lower than for hotspots prediction models 
...
ggplot(recall_data_base_all %>%
  filter(
    quantile %in% c(0.03, 0.05,0.1),
    cancer_type %in% c("blood","bone", "brain", "liver", "skin", "uterus")
    ), 
  aes(x=agg_level, fill=type, y=lift_recall)) + 
  geom_boxplot() + 
  facet_grid(quantile ~ cancer_type, scales = "free_y") + 
...
### Distribution of lift of precision for different probability quantiles
...
ggplot(recall_data_base_all %>%
  ...
  aes(x=agg_level, fill=type, y=lift_precision)) + 
  geom_boxplot() + 
  facet_grid(quantile ~ cancer_type, scales = "free_y") + 
  scale_fill_jco() + 
  theme(legend.position = "bottom") + 
...
rec_stats_all <- rec_stats %>%
  ...
  dcast(cancer_type + agg_level + quantile ~ type, value.var = "ci_lower_lift_recall")

rec_stats_all <- rec_stats_all %>%
  inner_join(
    rec_stats_bkpt, by=c("cancer_type","quantile")) %>%
  mutate(
    ratio_hsp_to_bkpt =  base / bkpt,
    ratio_hspvsbkpt_to_bkpt =  hsp_vs_bkpt / bkpt,
    ratio_hsp_to_hspvsbkpt =  base/ hsp_vs_bkpt
  )
...
g4 <- ggplot(rec_stats_all, aes(x=as.character(quantile), y=cancer_type, fill=ratio_hsp_to_bkpt)) + 
  geom_tile() + 
...
g5<- ggplot(rec_stats_all, aes(x=as.character(quantile), y=cancer_type, fill=ratio_hsp_to_hspvsbkpt_bucket)) + 
  geom_tile() + 
  facet_grid(~agg_level, switch = "x")+ 
  scale_fill_gradientn(colours = pal)+
  xlab("Probability percentile") + 
...
# ggsave(filename = "Fig5.tiff", plot = g_f, device = "tiff", path = "../paper_materials/", dpi = 600, units = "in", width = 7, height = 5, scale = 2, compression = "lzw")
...
# ggsave(filename = "Fig7a.tiff", plot = g_f1, device = "tiff", path = "../paper_materials/", dpi = 600, units = "in", width = 6, height = 5, scale = 2, compression = "lzw")
