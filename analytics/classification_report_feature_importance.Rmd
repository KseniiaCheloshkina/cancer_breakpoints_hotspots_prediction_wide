...
data_path <- "../data/output_third/classifier_results_by_feature_group/" 
...
roc_data_r <- roc_data %>%
  select("cancer_type", "agg_level", "feat_group",
...
# save train_by_feature_group.xlsx
# wb <- createWorkbook()
# addWorksheet(wb, "ROC_AUC")
# addWorksheet(wb, "Recall")
# writeData(wb, sheet="ROC_AUC", roc_data_r)
# writeData(wb, sheet="Recall", recall_stats_hsp)
# saveWorkbook(wb, "../data/output_third/reports/train_by_feature_group.xlsx", overwrite = T)


# save S15 - hotspots - full feature set
recall_stats <- recall_stats_hsp %>%
  select(c("quantile", "cancer_type", "agg_level", "feat_group","mean_recall", "mean_lift_recall", "ci_lower_lift_recall",
           "ci_upper_lift_recall",  "mean_precision","mean_lift_precision", "ci_lower_lift_precision", 
           "ci_upper_lift_precision")) %>%
  setNames(c("quantile", "Cancer type",		"Labeling type", "Feature group", "Mean recall",	"Mean lift of recall",
...
q3 <- roc_data_r %>%
  inner_join(
    rec_3, by=c("Cancer type","Feature group", "Labeling type")
  )
q5 <- roc_data_r %>%
  inner_join(
    rec_5, by=c("Cancer type", "Feature group", "Labeling type")
  )
...
# 
# saveWorkbook(wb, "../data/output_third/reports/s7.xlsx", overwrite = T)
```
...
```{r}
baseline_path <- '../data/output_third/reports/train_baseline.xlsx'
...
best_path <- '../data/output_third/reports/train_best_features_boruta_sign.xlsx'
...
baseline_data_path <- "../data/output_third/classifier_results_best_features_boruta_sign_all_agg_levels/"
...
all_roc_auc_data <- rbind.data.frame(
  roc_auc_data_base %>% select(cancer_type, agg_level, feature_group_name_proper, te_auc, tr_auc, pr_auc),
  roc_auc_data %>% select(cancer_type, agg_level, feature_group_name_proper, te_auc, tr_auc, pr_auc)
) %>%
  mutate(
...
pl_values <- rbind.data.frame(
  pl_values,
  all_roc_auc_data %>%
    select(cancer_type, agg_level, feature_group_name_proper, pr_auc) %>%
    setNames(c("cancer_type", "agg_level", "feature_group_name_proper", "value")) %>%
    mutate(
      metric = "PR AUC"
    )
...
g1
# ggsave(filename = "Fig3_main.tiff", plot = g1, device = "tiff", path = "../paper_third/", dpi = 600, units = "in", width = 5, height = 4, scale = 2, compression = "lzw")
```
```{r}
# write.csv(pl_values, file="../datasets/output_for_figures/fig3.csv")
```
...
g1
# ggsave(filename = "Fig9_supp.tiff", plot = g1, device = "tiff", path = "../paper_third/", dpi = 600, units = "in", width = 5, height = 4, scale = 2, compression = "lzw")
```

```{r}
agg_lev <- "99."

pl_values[pl_values$agg_level == agg_lev, ] %>%
  group_by(cancer_type, feature_group_name_proper, metric) %>%
  summarize(
    val = median(value),
    r = median(r)
  ) %>%
  filter(metric == "PR AUC") %>%
  arrange(cancer_type, desc(r))
...
breast_reg <- reg_data[(reg_data$cancer_type == "breast") & (reg_data$agg_level == "99."), ]
```
...
g_f <- ggarrange(g1, g2, g3,  
          ...
          font.label = list(size = 10, color = "black", face ="bold"))
g_f
# ggsave(filename = "Fig4_main_new.tiff", plot = g_f, device = "tiff", path = "../paper_materials/plots_after_review/", dpi = 600, units = "in", width = 5, height = 5, scale = 2.2, compression = "lzw")

```

```{r}
# write.csv(breast_sec_str, file="../datasets/output_for_figures/fig4_breast_sec_str.csv")
# write.csv(breast_tf, file="../datasets/output_for_figures/fig4_breast_tf.csv")
# write.csv(breast_reg, file="../datasets/output_for_figures/fig4_breast_reg.csv")
```
...
```{r}
# write.csv(df_onegroup_by_cancer_2, file="../datasets/output_for_figures/fig1_df_onegroup_by_cancer_2.csv")
# write.csv(df_onegroup_by_cancer_1, file="../datasets/output_for_figures/fig1_df_onegroup_by_cancer_1.csv")
```
