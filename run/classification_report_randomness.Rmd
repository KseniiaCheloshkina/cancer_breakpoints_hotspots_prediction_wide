...
ggplot(roc_auc_data_cur, aes(x=agg_level, y=te_auc, fill=agg_level)) + 
  geom_boxplot() + 
  scale_fill_jco() + 
  facet_wrap(~ cancer_type, nrow=2, scales="free_y") + 
...
```{r fig.height=9}
ggplot(roc_auc_data_base_all, aes(x=agg_level, y=te_auc, fill=type)) + 
  geom_boxplot() + 
...
ggplot(rec_stats_all, aes(x=as.character(quantile), y=cancer_type, fill=ratio_hsp_to_bkpt)) + 
  geom_tile() + 
...
ggplot(rec_stats_all, aes(x=as.character(quantile), y=cancer_type, fill=ratio_hsp_to_hspvsbkpt_bucket)) + 
  geom_tile() + 
  facet_grid(~agg_level, switch = "x")+ 
  scale_fill_gradientn(colours = pal)+
  theme(strip.placement = "outside")
